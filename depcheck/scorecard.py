...
from depcheck.depsize import build_size_report, BLOAT_THRESHOLD_MB, LARGE_PACKAGE_THRESHOLD_MB
...
def _score_size(project_path: str, result: ScanResult) -> CategoryScore:
    ...
    try:
        ...
        if size_report.bloated_packages:
            ...
            score.suggestions.append(
                f"Consider alternatives for bloated packages: "
...
def _score_maintenance(result: ScanResult) -> CategoryScore:
    ...
    if result.unmaintained_count > 0:
        ...
        score.suggestions.append(
            f"Replace {result.unmaintained_count} unmaintained packages with actively maintained alternatives"
        )
    else:
...
def generate_markdown_report(scorecard: ScorecardResult) -> str:
    ...
    lines = [
        ...
        f"## Category Scores",
        f"",
        f"| Category | Score | Weight | Weighted | Details |",
        f"|----------|-------|--------|----------|---------|",
    ]
    ...
    lines.append(f"| **Overall** | **{scorecard.overall_score:.0f}/100** | | **{scorecard.grade.value}** | |")
    ...
    lines.append("")
    lines.append(f"*Generated by [depcheck](https://github.com/jlaportebot/depcheck)*")
