...
def _sanitize_mermaid_id(name: str) -> str:
    """Sanitize a package name to a valid Mermaid node ID.
...
def _mermaid_status_style(status: str) -> str:
    """Get Mermaid style definition for a health status."""
    color = STATUS_COLORS.get(status, STATUS_COLORS.get("unknown", "#bdbdbd"))
    # Mermaid uses 'fill' for node background, 'color' for text
    return f"fill:{color},color:#fff"


def _mermaid_node_label(node: GraphNode) -> str:
    """Generate a node label for Mermaid diagram."""
    if node.version:
...
def _mermaid_node_shape(node: GraphNode) -> str:
    ...
    if node.status == "vulnerable":
        return "rect"  # Rectangle for vulnerable
    if node.status in ("yanked", "removed"):
        return "hexagon"  # Hexagon for yanked/removed
    if node.status == "unmaintained":
        return "diamond"  # Diamond for unmaintained
    return "rect"  # Default rectangle


# Add to_mermaid method to DependencyGraph class (monkey-patch for now,
# but we'll add it properly in the class definition)
# We'll add the method directly to the class below


# Add the method to DependencyGraph class
def _dependency_graph_to_mermaid(self) -> str:
    """Export the dependency graph as a Mermaid diagram.

    Generates a Mermaid flowchart (graph TD) with:
    - Nodes color-coded by health status
    - Version information in labels
    - Vulnerability count indicators
    - Proper Mermaid IDs (sanitized)

    Returns:
        A Mermaid diagram string ready for rendering.
    """
    lines = ["graph TD"]

    if not self.nodes:
        lines.append("    %% Empty graph - no dependencies found")
        return "\n".join(lines)
    ...
    # First pass: assign sanitized IDs to all nodes
    for node in self.nodes:
        mermaid_id = _sanitize_mermaid_id(node.id)
        # Handle collisions by appending a suffix
        original_id = mermaid_id
        counter = 1
        while mermaid_id in node_id_map.values():
            mermaid_id = f"{original_id}_{counter}"
            counter += 1
        node_id_map[node.id] = mermaid_id

    # Define all nodes
    for node in self.nodes:
        mermaid_id = node_id_map[node.id]
        label = _mermaid_node_label(node)
        # Escape quotes in label
        label_escaped = label.replace('"', '\\"')
        style = _mermaid_status_style(node.status)

        # Add vulnerability indicator to label
        if node.vuln_count > 0:
            label_escaped += f" ⚠{node.vuln_count}"

        lines.append(f'    {mermaid_id}["{label_escaped}"]')
        lines.append(f"    style {mermaid_id} {style}")
        defined_nodes.add(mermaid_id)

    # Define all edges (links)
    ...
    # Add clickable links for packages with versions (optional)
    # This is a comment for documentation
    lines.append("")
    lines.append("    %% Generated by depcheck")

    return "\n".join(lines)


# Monkey-patch the method onto the class
DependencyGraph.to_mermaid = _dependency_graph_to_mermaid


def write_graph_mermaid(
    graph: DependencyGraph,
